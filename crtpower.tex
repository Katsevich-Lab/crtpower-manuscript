...
\begin{abstract}
For testing conditional independence (CI) of a response $Y$ and a predictor $X$ given covariates $Z$, the recently introduced model-X (MX) framework has been the subject of active methodological research, especially in the context of MX knockoffs and their successful application to genome-wide association studies. In this paper, we study the power of MX CI tests, yielding quantitative explanations for empirically observed phenomena and novel insights to guide the design of MX methodology. We focus our analysis on the conditional randomization test (CRT), \color{red} whose validity conditional on $Y,Z$ allows us to view it as a test of a point null hypothesis involving the conditional distribution of $X$. We use the Neyman-Pearson lemma to derive the most powerful CRT statistic against a point alternative \color{black} as well as an analogous result for MX knockoffs. \color{red} We define CRT-style analogs of $t$- and $F$-tests with explicit critical values, \color{black} and show that they have uniform asymptotic Type-I error control under the assumption that \textit{only the first two moments of $X$ given $Z$ are known}, a significant relaxation of MX. \textcolor{red}{Also asymptotic equivalence to the CRT.} Finally, we derive expressions for the power of these tests against local semiparametric alternatives, explicitly capturing the prediction error of the underlying learning algorithm. 
\end{abstract}
...
\begin{center}
	...
	\end{minipage}
\end{center}
\color{red} Note that a one-sided version of this test (the \textit{MX(2) $t$-test}) can be defined for $d = 1$ by rejecting for large values of $U_n(\srx, \sry, \srz)$. \color{black}
...
\begin{figure}[h!]
	\includegraphics[width = \textwidth]{../simulation/unconditional.png}
	\caption{Unconditional distribution of $U_n(\srx, \sry, \srz)$ across the nine simulation settings.}
	\label{fig:unconditional}
\end{figure}

\begin{figure}[h!]
	\includegraphics[width = \textwidth]{../simulation/conditional.png}
