...
\usepackage[uniquename=false, uniquelist = false, url = false, doi = false, isbn = false, sorting = none, natbib = true, backend = bibtex, maxbibnames=10]{biblatex}

\addbibresource{~/.references/crtpower}
% \addbibresource{crtpower}
...
\color{red}
To do for this revision:
\begin{itemize}
	\item Include the example of “learn g-hat on observational data, test CI on experimental data”
	\item  Move 3.4 to appendix
	\item smooth last paragraph of 3 to section 4
\end{itemize}
...
%rejects with probability \[\gamma \equiv \frac{\alpha - \mathbb {P}[T(\srxk, \sry, \srz) > C(\sry,\srz)|\sry,\srz]}{\mathbb {P}[T(\srxk, \sry, \srz) = C(\sry,\srz)|\sry,\srz]}\] in cases when the test statistic is exactly equal to the rejection threshold. Note that the above probabilities in the definition of $\gamma$ are over the resampling distribution $\srxk\ |\ \sry, \srz$. 
In practice, the threshold $C(\sry,\srz)$ is approximated by computing $T(\srxk^b, \sry, \srz)$ for a large number $B$ of Monte Carlo resamples $\srxk^b \sim \srx|\srz$. % For $b = 1,\dots, B$, a resample $\srxk^b$ is drawn and the statistic $T(\srxk^b, \sry, \srz)$ is recomputed. Then, the CRT threshold is obtained through the empirical quantile
%\begin{equation}
%	\widehat C(\sry, \srz) \equiv Q_{1-\alpha}\left\{T(\srx, \sry, \srz), \{T(\srxk^b, \sry, \srz)\}_{b = 1, \dots, B}\right\}.
%	\label{empirical-quantile}
%\end{equation}
%If desired, a size $\alpha$ test can be obtained by computing $\gamma$ in the spirit of definition~\eqref{empirical-quantile}, but usually the CRT is implemented as a non-randomized test rejecting when $T(\srx, \sry, \srz) > \widehat C(\sry, \srz)$. 
%The randomization $\gamma$ is not applied in practice, the test rejecting when $T(\srx, \sry, \srz) > \widehat C(\sry, \srz)$. The finite-sample validity of the CRT for any $B \leq \infty$ and any $n,d,r,p$ follows from the fact that the resampled triples $(\srxk, \sry, \srz)$ are exchangeable with the original $(\srx, \sry, \srz)$ under the null, much like the argument used for a permutation test. 
...
\begin{center}
	\begin{minipage}{\linewidth}
		\begin{algorithm}[H]
			...
			Obtain $\widehat g_n$ by fitting $g$ (either in-sample, out-of-sample, or a mix of both)\;
...
% The power of CRT with $\widehat g_n$ fit in-sample is a more subtle object to analyze, as it may depend on the degree to which $\widehat g_n$ overfits. 
...
% while our result holds for any $\widehat g_n$. 
% and it would be interesting whether expression~\eqref{eq:main-conclusion} holds for more general classes of test statistics. 
...
\section{\textcolor{blue}{Most powerful} one-bit $p$-values for knockoffs}
\label{sec:knockoffs}
...
\begin{theorem} \label{prop:knockoff-optimality}
	...
	\end{equation}
	we have that the above likelihood statistic yields \textcolor{blue}{an optimal one-bit $p$-value:}
	\begin{equation}
...
We thank Asaf Weinstein, Timothy Barry, and Stephen Bates for detailed comments on earlier versions of the manuscript, as well as Ed Kennedy and Larry Wasserman for discussions of the connections to causal inference. \textcolor{blue}{We also thank two anonymous referees for constructive feedback that greatly helped us improve the manuscript.}
...
\begin{proof}
Let $\phi$ be any test satisfying conditional validity property~\eqref{eq:conditional-validity}. \color{blue}Let $\mathcal A$ be a set of pairs $(\sfy, \sfz)$, for which both $\phi$ and $\phi^{\CRT}_{T^{\textnormal{opt}}}$ have level $\alpha$ conditionally on $\sry = \sfy, \srz = \sfz$. By assumption, $\mathbb P[(\sry, \srz) \in \mathcal A] = 1$. \color{black} Now, fix realizations $(\sfy, \sfz) \in \mathcal A$. We first claim that the conditional power of $\phi$ is bounded above by that of $\phi^{\CRT}_{T^{\textnormal{opt}}}$, i.e.
	\begin{equation}
	...
	\end{equation}
	verifying the conditional optimality claim~\eqref{conditional-optimality}. \color{blue}To obtain the unconditional claim~\eqref{unconditional}, we take an expectation over $\sry, \srz$ and use the fact that $\mathbb P[(\sry, \srz) \in \mathcal A] = 1$:
	\begin{equation}
		\begin{split}
...
%	\begin{equation*}
%	\mathbb P_{\bar{\mathcal L}}[T^{\textnormal{opt}}(\srx, \sry, \srz) = C(\sry,\srz) \mid \sry,\srz] = 0,
%	\end{equation*}
